...
### ⚠️ Authentication — this endpoint is PUBLIC (no JWT)

Per the team-lead's directive, the option-chain endpoint has **NO
authentication**. The other `/market-data/*` endpoints (historical, tick,
replay) still require a Bearer JWT — only the option-chain endpoint is
public. In Swagger UI, this endpoint will NOT show a lock icon.
...
> **Alternative:** There is also a standalone no-auth FastAPI app at
> `app/standalone_truedata.py` that keeps a single long-lived `TD_live`
> websocket open and serves option-chain + tick data via GET endpoints
> on port 8086. See "Standalone TrueData option-chain API" below.
...
**Note:** This endpoint does NOT return 401 — it is public (no JWT).

### Example (curl)

```bash
# NO JWT NEEDED — this endpoint is public.
#
# Capture dual-expiry NIFTY option chains: 30s window, 5s snapshots,
...
│       ├── users.py             # /users/me
│       ├── market_data.py       # TrueData historical + tick + replay (JWT) + option-chain (PUBLIC, no JWT) endpoints
...
├── main.py                      # FastAPI app factory (main app, port 8000)
└── standalone_truedata.py       # Standalone NO-AUTH app (port 8086) — long-lived TD_live + GET endpoints + Excel exports
```

The main app (`app/main.py`) runs on port 8000. Auth is required for
all `/market-data/*` endpoints EXCEPT `/option-chain/export` which is
PUBLIC (no JWT). The standalone app (`app/standalone_truedata.py`)
runs on port 8086 with NO auth and serves option-chain + tick data via
GET endpoints — see "Standalone TrueData option-chain API" below.
...
- Option-chain export (PUBLIC — no auth; validation — empty chains, too many chains, chain_length bounds, duration bounds, duplicate pairs — hard-fail incl. trial's "User Subscription Expired", happy path with dual expiry, greek columns inclusion, underlying upper-casing, service-level edge case)
...
In addition to the main app's `POST /api/v1/market-data/truedata/option-chain/export`
endpoint (which is public, no JWT — see above), there is also a standalone
FastAPI app at `app/standalone_truedata.py` that matches the team-lead's
reference script exactly: a single long-lived `TD_live` websocket (one
connection per process), tick callbacks that capture every trade +
bidask, and a 30s daemon-thread auto-save loop.
...
Reads `.env` via `python-dotenv` (same `TRUEDATA_USERNAME` /
`TRUEDATA_PASSWORD` / `TRUEDATA_LIVE_PORT` as the main app, plus optional
`API_PORT` default 8086 and `AUTO_SAVE_INTERVAL_SECONDS` default 30).

### Endpoints (all public, no JWT, port 8086)
...
Preset chains + symbols are code-level config at the top of
`app/standalone_truedata.py` (edit `PRESET_CHAINS` and `MY_SYMBOLS`
to add BANKNIFTY/FINNIFTY/etc.). Excel exports land in
`option_chain_exports/` and `tick_data_exports/` (both gitignored).
