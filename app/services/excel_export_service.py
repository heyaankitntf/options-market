...
# ---------------------------------------------------------------------------
# The build_option_chain_zip() + _build_option_chain_metadata_text()
# helpers have been removed along with the JWT-protected POST endpoint
# they served. Option-chain access is now served by the standalone
# no-auth FastAPI app in `app/standalone_truedata.py` (port 8086), which
# writes Excel files directly to disk via pandas/openpyxl instead of
# streaming a ZIP back over HTTP. See the "Standalone TrueData
# option-chain API" section in the README.
