...
Exposes four endpoints:
    ...
    POST /api/v1/market-data/truedata/option-chain/export
        [REMOVED] — option-chain access is now served by the standalone
        no-auth app in `app/standalone_truedata.py` (port 8086). See the
        "Standalone TrueData option-chain API" section in the README.

All authenticate via the same Bearer JWT dependency used by `/users/me`,
write one legacy `.xls` file per requested symbol, and stream the
...
# endpoint has been removed. Option-chain access is now served by the
# standalone no-auth FastAPI app in `app/standalone_truedata.py`, which
# keeps a single long-lived TD_live websocket (one connection per process)
# and exposes GET endpoints for chain data, tick data, and Excel exports.
#
# Why the move?
#   1. The old endpoint opened a fresh TD_live per request, which re-triggers
#      the SDK's 'User Subscription Expired' dance on every call for trial
#      accounts. The standalone opens ONE connection at startup and keeps
#      it alive, eliminating that overhead.
#   2. The old endpoint required JWT auth, which made it unusable from
#      simple scripts / curl / browser. The standalone has NO auth — it
#      runs on a separate port (8086) so it can be firewall-restricted
#      independently of the main API.
#   3. The standalone matches the team-lead's reference script exactly,
#      including the bidask/trade callbacks that capture ticks to
#      `tick_data_exports/` and the 30s auto-save loop.
#
# Run it with:
