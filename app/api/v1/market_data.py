...
All endpoints authenticate via the same Bearer JWT dependency used by
`/users/me`, EXCEPT the option-chain endpoint which is PUBLIC (no JWT)
per the team-lead's directive. All write one legacy `.xls` file per
requested symbol/chain and stream the bundle back as an
...
def export_truedata_option_chain_xls(
    ...
    logger.info(
        "TrueData option-chain export requested (no auth) — "
    ...
    # No current_user — this endpoint is public (no JWT).
    zip_name = f"truedata_option_chain_{duration}s.zip"
